            ...
            <div class="phase-detail">
              <div class="detail-section">
                ...
                <div class="detail-content">
                  After the documents are uploaded, the LLM analyzes the content and generates an ontology suited to simulation work, including entity and relationship types.
                </div>
              </div>
              ...
              <div class="detail-section" v-if="projectData?.ontology">
                <div class="detail-label">生成的Edges类型 ({{ projectData.ontology.relation_types?.length || 0 }})</div>
                <div class="relation-list">
                  ...
                  <div v-if="(projectData.ontology.relation_types?.length || 0) > 5" class="relation-more">
                    +{{ projectData.ontology.relation_types.length - 5 }} 更多Edges...
                  </div>
                </div>
              </div>
          ...
          <div class="process-phase" :class="{ 'active': currentPhase === 1, 'completed': currentPhase > 1 }">
            ...
            <div class="phase-detail">
              <div class="detail-section">
                <div class="detail-label">API Notes</div>
                <div class="detail-content">
                  基于生成的本体，将文档分块后调用 Zep API 构建知识Graph，提取实体和Edges
                </div>
              </div>
              
              <!-- Waiting本体完成 -->
              <div class="detail-section waiting-state" v-if="currentPhase < 1">
                <div class="waiting-hint">Waiting for ontology generation完成...</div>
              </div>
...
const currentPhase = ref(-1) // -1: 上传中, 0: Ontology Generation中, 1: Graph Build, 2: 完成
...
const handleNewProject = async () => {
  ...
  try {
    ...
    error.value = '项目初始化失败: ' + (err.message || 'Unknown error')
  } finally {
    loading.value = false
  }
}
...
const startBuildGraph = async () => {
  try {
    ...
    buildProgress.value = {
      progress: 0,
      message: '正在启动Graph Build...'
    }
    
    const response = await buildGraph({ project_id: currentProjectId.value })
    
    if (response.success) {
      buildProgress.value.message = 'Graph Build任务已启动...'
      ...
      error.value = response.error || '启动Graph Build失败'
      buildProgress.value = null
    }
  } catch (err) {
    console.error('Build graph error:', err)
    error.value = '启动Graph Build失败: ' + (err.message || 'Unknown error')
    buildProgress.value = null
  }
}
