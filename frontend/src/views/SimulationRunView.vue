...
const handleGoBack = async () => {
  ...
  addLog('Preparing to return to Step 2. Closing the running simulation...')
  
  // 停止轮询
  stopGraphRefresh()
  
  try {
    // 先尝试优雅Close simulation environment
    ...
    if (envStatusRes.success && envStatusRes.data?.env_alive) {
      ...
      try {
        ...
        addLog(`Failed to close simulation environment，尝试Force stop...`)
        try {
          await stopSimulation({ simulation_id: currentSimulationId.value })
          addLog('✓ Simulation force-stopped')
        } catch (stopErr) {
          addLog(`Force stop失败: ${stopErr.message}`)
        }
      }
    } else {
      // 环境未运行，检查是否需要停止进程
    ...
    if (simRes.success && simRes.data) {
      ...
      try {
        ...
        if (configRes.success && configRes.data?.time_config?.minutes_per_round) {
          ...
          addLog(`Time config: each round is ${minutesPerRound.value} minutes`)
        }
      } catch (configErr) {
      ...
      addLog(`Loading simulation data失败: ${simRes.error || 'Unknown error'}`)
    }
  } catch (err) {
