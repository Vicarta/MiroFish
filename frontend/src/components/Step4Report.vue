...
const parseInsightForge = (text) => {
  ...
  try {
    ...
    // 提取Relationship Chains - 完整提取，不限制数量
...
const parsePanorama = (text) => {
  ...
  try {
    ...
    const activeMatch = text.match(/Current有效Facts:\s*(\d+)/)
    ...
    // 提取Current有效Facts - 完整提取，不限制数量
    const activeSection = text.match(/### 【Current有效Facts】[\s\S]*?\n([\s\S]*?)(?=\n###|$)/)
    if (activeSection) {
    ...
    // 提取Entities Involved - 完整提取，不限制数量
    ...
    return () => h('div', { class: 'insight-display' }, [
      ...
      h('div', { class: 'insight-content' }, [
        ...
        activeTab.value === 'subqueries' && props.result.subQueries.length > 0 && h('div', { class: 'subqueries-panel' }, [
          h('div', { class: 'panel-header' }, [
            h('span', { class: 'panel-title' }, 'Drift-query generated analytical sub-questions'),
...
// 提取Final Answer内容 - 从 LLM response 中提取章节内容
