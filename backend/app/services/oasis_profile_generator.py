...
class OasisProfileGenerator:
    ...
    def _get_system_prompt(self, is_individual: bool) -> str:
        """获取系统提示词"""
        base_prompt = "You are an expert social-media persona designer. Generate detailed, realistic personas for simulation work while staying close to the source material. You must return valid JSON, and string values must not contain unescaped newlines. Use English."
        return base_prompt
...
6. country: Country name (use English, for example "China")
7. profession: 职业
...
- persona必须是一段连贯的文字描述
- Use English for all fields except that the gender field must still use the English values "male" or "female"
- 内容要与实体信息保持一致
...
- Use English for all fields except that the gender field must still use the English value "other"
