...
INTERVIEW_PROMPT_PREFIX = "Based on your persona, memories, and prior actions, reply directly in plain text without calling any tools: "
...
def prepare_simulation():
    ...
    步骤：
    1. 检查是否Existing prepared data found
    2. 从Zep图谱读取并过滤实体
    ...
    返回：
        {
            ...
            "data": {
                ...
                "message": "Preparation task started | existing prepared data found",
    ...
    try:
            ...
            if is_prepared:
                ...
                return jsonify({
                    ...
                    "data": {
                        ...
                        "status": "ready",
                        "message": "Existing prepared data found. No need to regenerate",
        ...
        logger.error(f"启动准备Task failed: {str(e)}")
        return jsonify({
            "success": False,
            "error": str(e),
            "traceback": traceback.format_exc()
...
def get_prepare_status():
    ...
    2. 通过simulation_id检查是否Existing prepared data found
            ...
            if is_prepared:
                return jsonify({
                    ...
                    "data": {
                        ...
                        "status": "ready",
                        "progress": 100,
                        "message": "Existing prepared data found",
                        "already_prepared": True,
                        "prepare_info": prepare_info
                    }
                })
...
def download_simulation_config(simulation_id: str):
    ...
    try:
        ...
        if not os.path.exists(config_path):
            return jsonify({
                "success": False,
                "error": "Configuration file not found. Call /prepare first"
            }), 404
...
def download_simulation_script(script_name: str):
    ...
    try:
        ...
        if script_name not in allowed_scripts:
            return jsonify({
                "success": False,
                "error": f"Unknown script: {script_name}，可选: {allowed_scripts}"
            }), 400
...
def start_simulation():
    ...
    try:
        ...
        if platform not in ['twitter', 'reddit', 'parallel']:
            return jsonify({
                "success": False,
                "error": f"Invalid platform type: {platform}，可选: twitter/reddit/parallel"
            }), 400
        ...
        if enable_graph_memory_update:
            ...
            if not graph_id:
                return jsonify({
                    "success": False,
                    "error": "Graph memory updates require a valid graph_id. Make sure the project graph is built"
                }), 400
...
def interview_agent():
    ...
    try:
        ...
        if not SimulationRunner.check_env_alive(simulation_id):
            return jsonify({
                "success": False,
                "error": "The simulation environment is not running or has been closed. Make sure the simulation finished and is waiting for commands."
            }), 400
    ...
    try:
        ...
        for i, interview in enumerate(interviews):
            ...
            if item_platform and item_platform not in ("twitter", "reddit"):
                return jsonify({
                    "success": False,
                    "error": f"Interview list item {i+1} must use platform 'twitter' or 'reddit'"
                }), 400
